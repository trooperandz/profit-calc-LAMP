(function($) {
	...
	$.fn.dateZoom = function(options) {
		/* Only overwrites values that were explicitly passed 
		 * by the user in options
		 */
		var opts = $.extend($.fn.dateZoom.defaults, options);
		
		/* Loops through each matched element and returns 
		 * the modified jQuey object to maintain  chainability
		 */
	 ...
	 * plugin if a user is to inclined to use it
	 */
	$.fn.dateZoom.zoom = function(element, size, opts) {
		$(element).animate(	
				  ...
				  .clearQueue(); // Ensures only one animation occurrs
	};
})(jQuery);